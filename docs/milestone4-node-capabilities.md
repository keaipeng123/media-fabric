...
默认构建使用 `InMemorySipTransport` 支撑内部自测；Linux 目标环境可通过 `-DGB28181_ENABLE_PJSIP=ON` 打开实验性的 PJSIP UDP/TCP 监听 adapter。这个 adapter 已经能把 PJSIP 收到的 SIP 请求转换为统一的 `SipRequestContext`，再交给能力模块路由处理；能力模块发出的 `SipMessageContext` 可由传输层转换为出站 SIP 请求或请求上下文内的 stateless response。
...
当前能力模块已具备生命周期、运行上下文注入、SIP 路由注册、出站 SIP 消息生成能力。RTP 包解析、RTP payload 分片发送边界、RTP adapter 发送入口、RTP payload 按 marker 重组、PS pack/PES 基础解析、Annex-B H.264/H.265 NAL 识别、历史 `stream.file` 首帧读取、Annex-B 帧文件输出、媒体接收状态统计和 RTP 会话适配边界已迁入；真实网络抓包验证、持续文件/转流输出和 JRTPLIB 真实收发尚未完成。
...
- `BusinessQueryService` 提供协议无关的业务状态 JSON 查询边界，当前可输出 summary、Catalog peer 明细和 RecordInfo peer 明细，后续可挂接 HTTP/CLI 管理入口。
- `SipMessageContext` 已携带本地/远端地址、Expires 和 Digest challenge 参数，PJSIP adapter 可据此生成 REGISTER Expires、From/Contact 和 WWW-Authenticate。
- `SipRequestContext` 已携带 Digest Authorization 字段；PJSIP adapter 会把 Authorization header 解析为 username、realm、nonce、uri、response、algorithm、qop、nc、cnonce。
- `SipRequestContext` 已携带 Call-ID、CSeq 和 Contact 字段；PJSIP adapter 会从真实 SIP header 中提取这些字段。
- `SipRequestContext` 已携带 `ManscdpMessage` 字段；PJSIP adapter 收到 MESSAGE 时会解析 XML body 并用 root/CmdType 生成 `event`。
- `ManscdpMessage` 是轻量 MANSCDP XML 边界，当前提取 root、CmdType、SN、DeviceID、Result、StartTime、EndTime、SumNum，并解析 DeviceList/RecordList 下的 Item 明细。
...
- `StreamFileFrameSource` 是媒体层的历史测试流输入边界，当前读取旧 `stream.file` 中的 `StreamHeader + frame payload`，跳过非视频帧并返回视频帧数据。
...
- `MediaManager` 已维护 RTP 端口池和轻量媒体会话表；INVITE 成功后记录本地/远端 RTP、传输协议、方向、SSRC、Call-ID、INVITE CSeq 和远端 Contact，ACK 后确认会话。发送侧可把 Annex-B 帧封 PS 并通过 RTP adapter 分片发送；接收侧可对同 timestamp 的 RTP payload 按 marker 重组完整 PS，再解析 PS/PES/H.264/H.265，输出完整 Annex-B 帧并进入 `stream-receiving` 状态，BYE 后停止 RTP adapter 并释放端口。
- `TaskScheduler` 支持可唤醒停止，节点停止时先停周期任务，再停止能力模块，避免长间隔任务拖慢退出。
...
该开关依赖当前仓库 `3rd/lib` 中的 Linux PJSIP 静态库和系统 OpenSSL/ALSA。当前编辑环境不是 Linux，因此真实 PJSIP adapter 还需要在目标 Linux 开发环境完成编译、启动和抓包验证。
...
./build-linux-rtp/gb28181-server -c conf/gb28181-server.conf
```

该开关依赖 `3rd/lib/libjrtp.a` 和 `3rd/lib/libjthread.a`。当前仓库已有 `libjthread.a`，但缺少 `libjrtp.a`，因此真实 JRTPLIB 链接和 RTP 抓包验证需要在 Linux 环境补齐依赖后继续。
...
REGISTER Digest 自动重注册和 replay 拒绝抓包验证
Catalog/RecordInfo 的完整字段语义，以及把 `BusinessQueryService` 挂接到实际 HTTP/CLI 管理入口
INVITE/BYE 的 PJSIP dialog tags、ACK/BYE 真实抓包验证和错误响应兼容性验证
JRTPLIB adapter 的 Linux 链接、启动和真实 RTP 收发抓包验证
持续文件/转流输出，以及从 `StreamFileFrameSource` 循环驱动 `sendAnnexBFrame`
```

这些内容需要在 Linux 目标环境里逐条迁移和抓包验证。
