...
{
    EventPoll eventPoll;
    if(eventPoll.init(DefaultPollMethod())!=0)
    {
        return ST_SYSERROR;
    }
    ...
    while(true)
    {
        ...
        if(status==ST_OK)
        {
            for(size_t i=0;i<pollEvents.size();i++)
            {
                ...
                //非阻塞 connect 进入 EINPROGRESS 后，单看 socket 变可写还不够，必须再查一次 SO_ERROR。因为“可写”既可能表示连接成功，也可能表示连接失败但状态已经可取
...
StatusType ECSocket::createConnByPassive(int localport,int* lsockfd,int* connfd,int* timeout)
{
    ...
    *connfd=-1;
    ...
    std::cerr<<"start tcpserver localport="<<localport<<std::endl;
    ...
    std::cerr<<"tcpserver listening localport="<<localport<<std::endl;
    ...
    while(true)
    {
        ...
        std::cerr<<"pollEvents.SIZE()"<<pollEvents.size()<<std::endl;
        if (status==ST_OK)
        {
            for(size_t i=0;i<pollEvents.size();i++)
            {
                if(pollEvents[i].sockfd==sockfd)
                {
                    ...
                    if(acceptedFd!=-1)
                    {
                        std::cerr<<"EVENT TYPE:"<<pollEvents[i].outEvents<<std::endl;
                        *lsockfd=sockfd;
                        *connfd=acceptedFd;
                        return ST_OK;
                    }
...
StatusType ECSocket::createConnByActive(int localPort,string dspip,int dstport,int* connfd,int* timeout)
{
    ...
    *connfd=-1;
    ...
    std::cerr<<"tcpclient connect localport="<<localPort<<" remote="<<dspip<<":"<<dstport<<std::endl;
    ...
    while(true)
    {
        ...
        if(timeout==NULL)
        {
            ...
            if(ret<0)
            {
                std::cerr<<"connect error errno="<<errno<<" msg="<<strerror(errno)<<" localport="<<localPort<<" remote="<<dspip<<":"<<dstport<<std::endl;
                close(sockfd);
                return ST_SYSERROR;
            }
            *connfd=sockfd;
        ...
        if(ret<0&&errno!=EINPROGRESS)//立刻失败，而且不是“连接进行中”
        {
            ...
            errno=connectErr;
            std::cerr<<"connect error errno="<<connectErr<<" msg="<<strerror(connectErr)<<" localport="<<localPort<<" remote="<<dspip<<":"<<dstport<<std::endl;
            return ST_SYSERROR;
        }
        ...
        {
            std::cerr<<"restore socket flags error"<<std::endl;
            close(sockfd);
            return ST_SYSERROR;
        }
        ...
        else
        {
            ...
            std::cerr<<"connect error after wait errno="<<connectErr<<" msg="<<strerror(connectErr)<<" localport="<<localPort<<" remote="<<dspip<<":"<<dstport<<std::endl;
        }
        close(sockfd);
        return status;
    }
